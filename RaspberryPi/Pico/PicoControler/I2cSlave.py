...
class i2c_slave:
    ...
    # List Of Registers
    IC_CON = 0x00 #I2C Control Register [10:0]
    IC_TAR = 0x04 #I2C Target Address Register
    IC_SAR = 0x08 #I2C Slave Address Register, default 0x055
    IC_DATA_CMD = 0x10 #I2C Rx/Tx Data Buffer and Command Register
    IC_RAW_INTR_MASK = 0x30 # I2C Raw Interrupt Status Register
    ...
    def __init__(self, i2cID = 0, sda=0, scl=1, slaveAddress=0x41):
        ...
        self.set_reg(self.IC_CON, 0x04)   # 7bit-adrs, 400kbps
        #
        # self.clr_reg(self.IC_RAW_INTR_MASK, 0x8ff)
        # self.set_reg(self.IC_RAW_INTR_MASK, 0x8df)   # 7bit-adrs, 400kbps
    ...
    def intr_reg_read(self):
        # print("RD_REQ = %d", hex(mem32[ self.i2c_base | self.IC_RAW_INTR_MASK]))
        print("RD_REQ = %d", mem32[ self.i2c_base | self.IC_RAW_INTR_STAT] & 0x20)


    def anyRead(self):
        status = mem32[ 0x40048034 ] & 0x20
        if status :
            return True
        return False
