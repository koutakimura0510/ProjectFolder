...
create_clock -name PCLK -period 40.000 [get_ports {GPIO_0[0]}]
...
set_input_delay  -clock [get_clocks $PLL0_SYSCLK] 1 \
  [remove_from_collection [all_inputs] {GPIO_0*}]
set_input_delay  -clock {PCLK}  1 [get_ports {GPIO_0*}]
set_output_delay -clock [get_clocks $PLL0_SYSCLK] 1 \
  [remove_from_collection [all_outputs] {DRAM_CLK VGA*}]
set_output_delay -clock {PCK} -max 1 [get_ports {VGA*}]
set_output_delay -clock {PCK} -min 0 [get_ports {VGA*}]
