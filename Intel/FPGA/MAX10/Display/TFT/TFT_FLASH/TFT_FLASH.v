 ...
 * -------------------------------------------------------------------
 * TFT-LCDサンプルソース
 * FPGAでTFT_LCD240x320の初期設定を行い、全画面点灯を行う
 * 18bit CPU パラレルモードで動作
 * -------------------------------------------------------------------
 * Quartus Prime: 20.1
 * Ubuntu LTS   : 20.04
 * Simlation    : Model Sim
...
parameter WAIT_TIME  = 500000;


/**------------------------------------------
 * reg・wire宣言
 *------------------------------------------*/
wire wait_100ms, cnt_1sec;
reg [25:0] tmp_count;
reg rd, wrx, dcx, enable;
reg [1:0] state;
reg [6:0] cnt;
...
initial begin
    ...
    tft_init[3] = {TFT_DATA_BYTE, RGB_18bit_MODE};
    ...
    tft_init[14] = {TFT_DATA_BYTE, 8'h90};
    ...
    tft_init[50] = {TFT_DATA_BYTE, SYSTEM_INTERFACE};
    tft_init[51] = {TFT_CMD_BYTE,  COLUMN_ADDRESS_SET};
    tft_init[52] = {TFT_DATA_BYTE, 8'h00};
    tft_init[53] = {TFT_DATA_BYTE, 8'h00};
    tft_init[54] = {TFT_DATA_BYTE, 8'h00};
    tft_init[55] = {TFT_DATA_BYTE, TFT_DISPLAY_WIDTH};
    tft_init[56] = {TFT_CMD_BYTE,  PAGE_ADDRESS_SET};
    tft_init[57] = {TFT_DATA_BYTE, 8'h00};
    tft_init[58] = {TFT_DATA_BYTE, 8'h00};
    tft_init[59] = {TFT_DATA_BYTE, TFT_DISPLAY_HEIGHT >> 8};
    tft_init[60] = {TFT_DATA_BYTE, TFT_DISPLAY_HEIGHT & 8'hff};
...
always @ (posedge CLK) begin
	if (RESET == 1'b1) begin
		tmp_count <= 26'b0;
    end else if (cnt_1sec == 1'b1) begin
		tmp_count <= 26'b0;
    end else begin
...
always @(posedge CLK) begin
    if (RESET == 1'b1) begin
        ...
        if (state == 2'b0) begin
            ...
            wrx <= 1;
            state <= state + 1;
        end else if (wait_100ms == 1'b1) begin
            cnt <= cnt + 1;
            enable <= 0;
            state  <= 0;
        end
...
assign cnt_1sec   = (tmp_count == (SYSTEM_CLK - 1)) ? 1'b1 : 1'b0;
assign wait_100ms = (tmp_count[18:0] == (WAIT_TIME - 1)) ? 1'b1 : 1'b0;
assign LEDR       = cnt;
assign GPIO[18]   = rd;
assign GPIO[20]   = wrx;
assign GPIO[22]   = dcx;
assign GPIO[7:0]  = (enable == 1'b1) ? tft_init[cnt][7:0] : 8'b00000000;
