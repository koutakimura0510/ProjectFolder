...
module SpiMaster (
	...
	input  iSpiEn,			// 
	input  iCsOutCtrl,		// 
	output oSpiIntr,		// 1byte トランザクションの完了
	// CLK Reset
	input  iRST,
	input  iCLK
);
...
//----------------------------------------------------------
// FPGA Master Side
// 1byte 送信のみ対応しており、4byte(32bit)などのデータを送受信する場合は、
// 上位モジュールで対応を行う。
...
begin
	...
	// Sck の立ち下がり回数カウント
	casex ({iRST, iSpiEn, iDivCke, rSck})
	...
	casex ({iRST, iSpiEn, qHoldTimeCke, rHoldTimeState[0:0], iDivCke, rSck})
...
assign oSpiSck	  	= rSck;
assign oSpiMosi  	= rMosi[7];
assign oSpiCs	  	= iCsOutCtrl;
