...
static void sdl_finish(SDL_Window **window, SDL_Renderer **renderer, SDL_Texture **player_tx);
...
static void map_draw(SDL_Renderer *renderer, SDL_Texture **map_tx, MapchipWork *chip);
...
static void map_draw(SDL_Renderer *renderer, SDL_Texture **map_tx, MapchipWork *chip)
{
	...
	for (uint32_t i = 0; i < MAP_DRAW_HEIGHT; i++)
	{
		...
		{
			map_data = chip->fbuf_map[((i + y) * maxwidth) + j + x];
			obj_data = chip->fbuf_obj[((i + y) * maxwidth) + j + x];
 ...
 * @retval None
 */
static uint8_t map_config_key(MapchipWork *chip, int32_t key)
{
	switch (key)
	{
 ...
 */
static uint8_t region_config_key(MapchipWork *chip, int32_t key)
{
	/*
	 *
	 * 何かのキーで桁数を選択できるようにして
	...
	case SDLK_RIGHT: xpos_move_right(chip); break;
	case SDLK_q: xpos_move_right(chip); break;
	case SDLK_a: chip->region.digit = 1; break;
	case SDLK_s: chip->region.digit = 2; break;
	case SDLK_d: chip->region.digit = 3; break;
	...
	case SDLK_o: mapf_read(chip); break;
	case SDLK_p: mapf_write(chip); break;
	case SDLK_0: request_region_id(chip, 0); break;
	case SDLK_1: request_region_id(chip, 1); break;
	case SDLK_2: request_region_id(chip, 2); break;
	case SDLK_3: request_region_id(chip, 3); break;
	case SDLK_4: request_region_id(chip, 4); break;
	case SDLK_5: request_region_id(chip, 5); break;
	case SDLK_6: request_region_id(chip, 6); break;
	case SDLK_7: request_region_id(chip, 7); break;
	case SDLK_8: request_region_id(chip, 8); break;
	case SDLK_9: request_region_id(chip, 9); break;
...
int main(int argc, char *argv[])
{
	...
	if (false == sdl_create(&window, &renderer)) {
		sdl_finish(&window, &renderer, &player_tx);
		return 1;
	}
