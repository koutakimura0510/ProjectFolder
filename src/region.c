...
void region_write(MapchipWork *chip)
{
	...
	for (uint32_t y = 0; y < chip->diry_loop; y++)
	{
		...
		{
			chip->fbuf_reg[unit_pos + filed_pos + x + (y * chip->xsize)] = chip->region.id;
		}
	}
}
 ...
 * @retval None
 */
void request_region_id(MapchipWork *chip, uint32_t id)
{
	switch (chip->region.digit)
	{
	case 1:
		chip->region.id = id;
		chip->region.key_count = 0;
		break;

	case 2:
		if (chip->region.key_count == 0)
		{
			chip->region.id = id;
			chip->region.key_count = 1;
		}
		else
		{
			chip->region.id = (chip->region.id * 10) + id;
			chip->region.key_count = 0;
		}
		break;
	...
	case 3:
		...
		{
			if (chip->region.key_count == 1)
			{
				chip->region.key_count = 2;
			}
			else
			{
				chip->region.key_count = 0;
