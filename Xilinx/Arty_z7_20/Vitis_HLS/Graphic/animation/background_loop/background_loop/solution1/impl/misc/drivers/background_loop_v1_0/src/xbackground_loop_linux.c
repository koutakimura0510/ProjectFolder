...
typedef struct {
    ...
    XBackground_loop_uio_map maps[ MAX_UIO_MAPS ];
} XBackground_loop_uio_info;

/***************** Variable Definitions **************************************/
static XBackground_loop_uio_info uio_info;
...
static int uio_info_read_name(XBackground_loop_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBackground_loop_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBackground_loop_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBackground_loop_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBackground_loop_Initialize(XBackground_loop *InstancePtr, const char* InstanceName) {
	XBackground_loop_uio_info *InfoPtr = &uio_info;
