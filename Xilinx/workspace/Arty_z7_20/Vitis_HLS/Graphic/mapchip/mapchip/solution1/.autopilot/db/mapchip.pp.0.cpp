...
u32 color_generate(u32 src, u32 dst, u8 alpha, u32 crip)
{
 u32 src_r = (src >> 16) & 0xff;
 u32 src_b = (src >> 8) & 0xff;
 u32 src_g = src & 0xff;
    u32 crip_r = (crip >> 16) & 0xff;
 u32 crip_b = (crip >> 8) & 0xff;
 u32 crip_g = crip & 0xff;
    u32 color;

    if ((crip_r == src_r) && (crip_b == src_b) && (crip_g == src_g)) {
        color = dst;
    }
    else
    {
  ...
  dst_g = ((alpha * src_g) + ((255 - alpha) * dst_g)) / 255;
        color = ((dst_r << 16) & 0xff0000) | ((dst_b << 8) & 0x00ff00) | (dst_g & 0xff);
    }
...
__attribute__((sdx_kernel("mapchip", 0))) void mapchip(
    volatile u32 *srcin,
    volatile u32 *dstin,
    volatile u32 *dstout,
...
{
#pragma HLS INTERFACE m_axi depth=307200 port=dstin offset=slave bundle=dst num_write_outstanding=16 max_write_burst_length=32
# 66 "mapchip.cpp"
...
#pragma HLS INTERFACE s_axilite port=crip
# 66 "mapchip.cpp"
...
# 84 "mapchip.cpp"
         ...
         dstout[x + y * frame_size] = color_generate(src[x], dst[x], alpha, crip);
        }
    }
}
