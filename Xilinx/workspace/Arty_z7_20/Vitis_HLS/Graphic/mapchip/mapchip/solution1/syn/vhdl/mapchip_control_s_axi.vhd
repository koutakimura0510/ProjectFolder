...
entity mapchip_control_s_axi is
generic (
...
port (
    ...
    crip                  :out  STD_LOGIC_VECTOR(31 downto 0);
    ap_start              :out  STD_LOGIC;
    ap_done               :in   STD_LOGIC;
    ap_ready              :in   STD_LOGIC;
    ap_idle               :in   STD_LOGIC
);
end entity mapchip_control_s_axi;
...
architecture behave of mapchip_control_s_axi is
    ...
    constant ADDR_CRIP_DATA_0               : INTEGER := 16#7c#;
    constant ADDR_CRIP_CTRL                 : INTEGER := 16#80#;
    ...
    signal int_crip            : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CRIP_DATA_0 =>
                        rdata_data <= RESIZE(int_crip(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    crip                 <= STD_LOGIC_VECTOR(int_crip);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CRIP_DATA_0) then
                    int_crip(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_crip(31 downto 0));
                end if;
            end if;
        end if;
    end process;
