...
# 2 "patbit/patbit.cpp" 2



__attribute__((sdx_kernel("patbit", 0))) void patbit(
...
{
#pragma HLS INTERFACE m_axi depth=307200 port=dstout offset=slave num_write_outstanding=16 max_write_burst_length=16
# 13 "patbit/patbit.cpp"
...
#pragma HLS TOP name=patbit
# 13 "patbit/patbit.cpp"

    height_loop: for (int y = 0; y < height; y++) {
#pragma HLS LOOP_TRIPCOUNT min=1 max=480 avg=240
# 14 "patbit/patbit.cpp"

        width_loop: for (int x = 0; x < width; x++) {
#pragma HLS LOOP_TRIPCOUNT min=1 max=640 avg=320
# 15 "patbit/patbit.cpp"

            dstout[(x + xpos) + (y + ypos) * 640] = color;
        }
    }
}
