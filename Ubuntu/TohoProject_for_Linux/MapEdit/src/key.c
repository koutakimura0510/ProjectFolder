...
void xpos_move_right(MapchipWork *chip)
{
	...
	if ((MAP_DRAW_WIDTH + p->field_xpos) >= chip->xsize) {
		p->unit_xpos++;
		return;
	}
...
void ypos_move_down(MapchipWork *chip)
{
	...
	if ((MAP_DRAW_HEIGHT + p->field_ypos) >= chip->ysize) {
		p->unit_ypos++;
		return;
	}
...
void map_direct(MapchipWork *chip)
{
	...
	switch (p->unit_direction) {
		case RIGHT:
			if (chip->xsize <= (p->field_xpos + p->unit_xpos)) {
				p->unit_xpos--;
			}
			break;
		...
		case DOWN:
			if (chip->ysize <= (p->field_ypos + p->unit_ypos)) {
				p->unit_ypos--;
			}
			break;
