...
int main(int argc, char **argv)
{
	...
	for (uint32_t i = 0; i < 6; i++) {
		fread(&status, sizeof(t_status), sizeof(status) / sizeof(status), fp);
		printf("%s, HP:%d, MP:%d, レベル:%d, 強さ:%d, 素早さ:%d, 賢さ:%d, 経験値:%d\n",
		status.name, status.hp, status.mp, status.level, status.strength, 
		status.speed, status.wisdom, status.expr);
	}
